<?php 
    ...
    if(isset($_POST['submit']))
    {
      //Kiem tra
      if(empty($_POST['masv']))
        $errMSSV = "* Chưa nhập vào mssv";

      if(empty($_POST['hoten']))
        $errHoTen = "* Chưa nhập vào tên";

      if(empty($_POST['ngaysinh']))
        $errNgaySinh = "* Chưa nhập vào ngày sinh";

      if(empty($_POST['email']))
        $errEmail = "* Chưa nhập vào email";
      
      if(empty($errMSSV) && empty($errHoTen) && empty($errNamSinh) && empty($errEmail))
      {
        $SinhVien=array();
        $new_SV = array(
          'malop' => $lop,
...
<body>
  ...
  <div class="form-bt">
    ...
    <form class="form-tinhtoan" action="../views/bt_form.php?bai=8-config" method="POST" style="width:95%; margin-left:15px">
      <fieldset>
